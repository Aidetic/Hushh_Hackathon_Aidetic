...
class Agent:
    ...
    def handle_message(self, message, history=None, agenda=None):
        ...
        yield {"title": "logs", "content": section(f"Agent Plan for {self.username}", "🧠")}
        # yield format_plan(plan)
        yield {"title": "logs", "content": format_plan(plan)}
    ...
    def execute_plan(self, plan):
        ...
        for step_idx, step in enumerate(plan):
            ...
            yield {"title": "logs", "content": section(f"Executing {step_idx+1}", "🔎")}
            # yield f"**→ Executing:** {step}"
            yield {"title": "logs", "content": f"**→ Executing:** {step}"}
            ...
            if isinstance(response, types.GeneratorType):
                for substep in response:
                    ...
                    yield {"title": "logs", "content": " ".join(str(substep).splitlines())}
            ...
            yield {"title": "logs", "content": section(f"Result of Step {step_idx+1}", "📣")}
            # yield last_content
            yield {"title": "logs", "content": last_content}
